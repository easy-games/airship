...
namespace Adrenak.UniVoice.Networks {
    ...
    public class MirrorServer : IAudioServer<int> {
        ...
        /// <summary>
        /// Airship mod. List of clients which we discard audio input from (muted on server).
        /// </summary>
        public HashSet<int> ServerMutedClientIDs = new();
            ...
            if (tag.Equals(MirrorMessageTags.AUDIO_FRAME)) {
                ...
                foreach (var recipient in peersToForwardAudioTo) {
                    ...
                    if (ClientVoiceSettings.TryGetValue(recipient, out var recipientSettings)) {
                        ...
                        if (senderSettings != null && recipientSettings.mutedTags.Any(senderSettings.myTags.Contains))
                            continue;
                    }
        ...
        void OnServerConnected(int connId) {
            ...
            foreach (var peer in ClientIDs) {
                ...
                if (peer == connId) {
                    ...
                    SendToClient(connId, newClientPacket.Bytes, Channels.Reliable);
