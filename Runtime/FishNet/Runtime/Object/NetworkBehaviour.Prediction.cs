...
namespace FishNet.Object
{
    ...
    public abstract partial class NetworkBehaviour : MonoBehaviour
    {
        ...
        [FormerlySerializedAs("VerboseLogging")] public bool verboseLogging = false;
        ...
        private void Replicate_HandleReceivedReplicate<T>(int receivedReplicatesCount, T[] arrBuffer, BasicQueue<T> replicates, Channel channel) where T : IReplicateData
        {
            ...
            for (int i = 0; i < receivedReplicatesCount; i++)
            {
                ...
                if (tick > _lastReceivedReplicateTick)
                {
                    ...
                    if (verboseLogging && replicates.Count > 1)
                    {
                        Debug.Log($"Recv tick {arrBuffer[i].GetTick()} on time {Time.unscaledTime} localTick=" + TimeManager.LocalTick + ", replicates=" + replicates.Count);
                    }
