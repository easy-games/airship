...
/**
 * This is a modified version of Google's Firebase repo example: UIHandler.cs
...
namespace Assets.Code.FirebaseAPI
{
	...
	public class UIHandler : MonoBehaviour
	{
		...
		// Called when a sign-in without fetching profile data completes.
		void HandleSignInWithAuthResult(Task<AuthResult> task)
		{
			EnableUI();
			if (LogTaskCompletion(task, "Sign-in"))
			{
		...
		void GUIDisplayControls()
		{
			if (UIEnabled)
			{
				...
				{
					SignInWithProvider_DEPRECATED(MicrosoftAuthProvider.ProviderId);
				}
				if (GUILayout.Button("SignInWith | Yahoo"))
				{
					SignInWithProvider_DEPRECATED(YahooAuthProvider.ProviderId);
				}
				if (GUILayout.Button("ReauthWith | Microsoft"))
				{
					ReauthenticateWithProvider_DEPRECATED(MicrosoftAuthProvider.ProviderId);
				}
				if (GUILayout.Button("ReauthWith | Yahoo"))
				{
					ReauthenticateWithProvider_DEPRECATED(YahooAuthProvider.ProviderId);
				}
				if (GUILayout.Button("LinkWith | Microsoft"))
				{
					LinkWithProvider_DEPRECATED(MicrosoftAuthProvider.ProviderId);
				}
				if (GUILayout.Button("LinkWith | Yahoo"))
				{
					LinkWithProvider_DEPRECATED(YahooAuthProvider.ProviderId);
				}
				if (GUILayout.Button("Unlink User | Microsoft"))
				{
					UnlinkUser_DEPRECATED(MicrosoftAuthProvider.ProviderId);
				}
				if (GUILayout.Button("Unlink User | Yahoo"))
				{
					UnlinkUser_DEPRECATED(YahooAuthProvider.ProviderId);
				}
