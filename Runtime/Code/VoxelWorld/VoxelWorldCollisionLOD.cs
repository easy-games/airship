...
public class VoxelWorldCollisionLOD : MonoBehaviour {
    ...
    private void Awake() {
        ...
        _refreshDistanceSqr = Mathf.Pow(VoxelWorld.chunkSize / 2.0f, 2);
    }

    private void Start() {
        _voxelWorld.ChunkAdded += chunk => {
    ...
    private void CheckForLODUpdate(Vector3 currentPosition) {
        ...
        var min = currentPosition - distance * Vector3.one;
        var max = currentPosition + distance * Vector3.one;

        var currPosChunkSpace = VoxelWorld.WorldPosToChunkKey(currentPosition);
        var lastPosChunkSpace = VoxelWorld.WorldPosToChunkKey(_lastChunkUpdatePosition);
        ...
        for (var x = min.x; x < max.x + VoxelWorld.chunkSize; x += VoxelWorld.chunkSize) {
            for (var y = min.y; y < max.y + VoxelWorld.chunkSize; y += VoxelWorld.chunkSize) {
                for (var z = min.z; z < max.z + VoxelWorld.chunkSize; z += VoxelWorld.chunkSize) {
                    ...
                    var inLastRange = (chunkKey - lastPosChunkSpace).sqrMagnitude < ChunkDistanceSqr;
                    var inCurrRange = (chunkKey - currPosChunkSpace).sqrMagnitude < ChunkDistanceSqr;
