...
public class WorldSaveFile : ScriptableObject {
    ...
    public struct SaveChunk {
        ...
        public void Serialize(BinaryWriter writer, ushort version) {
            ...
            if (version >= 3) {
                writer.Write((uint)customData.Count);
        ...
        public static Vector3Int Deserialize(BinaryReader reader, ushort version, List<VoxelData> voxelData, List<uint> colors, Dictionary<ushort, BinaryBlob> customData) {
            ...
            if (version >= 3) {
                ...
                Log("Number of custom elements: " + numCustomElements);
                for (int i = 0; i < numCustomElements; i++) {
                    var voxelIndex = reader.ReadUInt16();
                    var arrayLength = reader.ReadUInt32();
                    Log("Loaded binary from save data. index: " + voxelIndex + " length: " + arrayLength);
    ...
    public void CreateFromVoxelWorld(VoxelWorld world) {
        ...
        foreach (var chunk in world.chunks) {
            ...
            if (!foundVoxel) {
                Debug.LogWarning("Trying to load empty chunk");
                continue;
            }

            
            Log("Valid Chunk: " + chunk.Value.chunkKey);
        ...
        Log("Writing chunk size: " + savedChunks.Count);
        foreach (var chunk in savedChunks) {
            Log("Serializing chunk: " + chunk.key);
        ...
        if (!Application.isPlaying) {
            Log($"Saved {finalChunkCounter} chunks to {name} (raw: {FormatDataSize(memStream.Length)}) (compressed: {FormatDataSize(chunksCompressed.Length)})");
        }
#endif
    }
    ...
    private void LoadChunkIntoVoxelWorld(VoxelWorld world, Dictionary<BlockId, BlockId> blockRemapping, Vector3Int key, IList<VoxelData> data, IList<uint> color, Dictionary<ushort, BinaryBlob> customData) {
        Log("Loading chunk into world: " + key);
    ...
    public void LoadIntoVoxelWorld(VoxelWorld world) {
        ...
        if (chunksCompressed != null && chunksCompressed.Length > 0) {
            ...
            Log("Loading non compressed chunk");
...
#if UNITY_EDITOR
        if (!Application.isPlaying) {
            Log($"[Voxel World]: Loaded {counter} chunks");
        }
#endif
