...
using UnityEngine;
//Todo: delete this, its from airshipSRP
[ExecuteInEditMode]
[LuauAPI]
public class MaterialColor : MonoBehaviour {
    ...
    public class ColorSetting {
        ...
        public float emissiveLevel = 0f;
        [NonSerialized]
        public String reference = "";


        public ColorSetting(Color materialColor, Color emissiveColor, float emissiveMix, float emissiveLevel) {
        ...
        public void CopyFrom(ColorSetting otherSettings) {
            ...
            this.emissiveLevel = otherSettings.emissiveLevel;
        }
    }
    ...
    public void DoUpdate() { 
        ...
        for (int i = 0; i < ren.sharedMaterials.Length; i++) {
            ...
            block.SetFloat("_EmissiveLevel", setting.emissiveLevel);
        }
    }
    ...
    private void RefreshVariables() {
        ...
        if (colorSettings.Count < ren.sharedMaterials.Length) {
            for (int i = colorSettings.Count; i < ren.sharedMaterials.Length; i++) {
                colorSettings.Add(new ColorSetting(Color.white, Color.white, 1.0f, 0.0f));
            }
        }
        if (colorSettings.Count > ren.sharedMaterials.Length) {
    ...
    public bool SetColorSettings(int materialIndex, ColorSetting settings) {
        RefreshVariables();
    ...
    public ColorSetting GetColorSettings(int materialIndex = 0) {
        if (materialIndex < 0 || materialIndex >= colorSettings.Count) {
            return new ColorSetting(Color.white, Color.black, 1, 0.0f);
        }
        ...
        if (targets.Length > 1) {
            ...
            for (int i = 0; i < max; i++) {
                ...
                foreach (MaterialColor targetObj in targets) {
                    ...
                    if (first == true) {
                        //Add a clone
                        originalValues.Add(new MaterialColor.ColorSetting(targetObj.colorSettings[i].materialColor, targetObj.colorSettings[i].emissiveColor, targetObj.colorSettings[i].emissiveMix, targetObj.colorSettings[i].emissiveLevel));
                        ...
                        setting.emissiveMix = EditorGUILayout.Slider("Emissive Mix", setting.emissiveMix, 0.0f, 1.0f);
                        setting.emissiveLevel = EditorGUILayout.Slider("Emissive Level", setting.emissiveLevel, 0.0f, 1.0f);
                        //dividing line
                        EditorGUILayout.LabelField("", GUI.skin.horizontalSlider);
                ...
                for (int i = 0; i < max; i++) {
                    ...
                    foreach (MaterialColor targetObj in targets) {
                        ...
                        if (hostObject == null) {
                            ...
                            if (originalValue.materialColor == newValue.materialColor && originalValue.emissiveColor == newValue.emissiveColor && originalValue.emissiveMix == newValue.emissiveMix && originalValue.emissiveLevel == newValue.emissiveLevel) {
                                break;
                            }
                        }
                        else {
                            ...
                            setting.emissiveLevel = hostSetting.emissiveLevel;
                        }
                    }
                }
